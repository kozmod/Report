...
//    public void      setDateCreate(Timestamp dateCreate) {this.dateCreate = dateCreate;}
    ...
    public void setSiteNumber(String siteNumber) {this.siteNumber.set(siteNumber);}

    public String getTypeHome()                    {return typeHome.get();}
    public StringProperty typeHomeProperty()       {return typeHome;}
    public void   setTypeHome(String typeHome) {this.typeHome.set(typeHome); }

    public double getSmetCost()                {return smetCost.get();}
    public DoubleProperty smetCostProperty()   {return smetCost;}
    public void   setSmetCost(double smetCost) {this.smetCost.set(smetCost);}

    public double         getSaleCost()        {return saleCost.get();}
    public DoubleProperty saleCostProperty()   {return saleCost;}
    public void setSaleCost(double saleCost)   {this.saleCost.set(saleCost);}
...
//        if (dateCreate            != null ? dateCreate.getTime() != that.dateCreate.getTime()         : that.dateCreate            != null) return false;
        if (siteNumber.getValue() != null ? !siteNumber.getValue().equals(that.siteNumber.getValue()) : that.siteNumber.getValue() != null) return false;
        if (typeHome.getValue()   != null ? !typeHome.getValue().equals(that.typeHome.getValue())     : that.typeHome.getValue()   != null) return false;
        if (smetCost.getValue()   != 0    ? !smetCost.getValue().equals(that.smetCost.getValue())     : that.smetCost.getValue()   != 0)    return false;
